...
function setTool(tool) {
    ...
    // Toggle form visibility based on tool
    if (tool === 'point') {
...
map.on('click', (e) => {
    ...
    if (currentTool === 'point') {
        ...
        if (!measureStart) {
            ...
            const line = L.polyline([measureStart, e.latlng], {color: '#dc3545', dashArray: '5, 10'})
                .addTo(measureLayers)
                .bindTooltip(formatDistance(map.distance(measureStart, e.latlng)), {permanent: true, direction: 'center'})
                .openTooltip(); 
            
            // Allow deletion on click
            line.on('click', function(evt) {
                L.DomEvent.stop(evt); // Prevent drawing a new point beneath the click
        ...
        if (!highlightStart) {
            highlightStart = e.latlng;
        } else {
            const bounds = [highlightStart, e.latlng];
            const comment = prompt("Label:") || "No comment";
            ...
            const label = L.marker(rect.getBounds().getCenter(), { 
                icon: L.divIcon({ className: 'highlight-label', html: `<div style="background:white; color:black; border-radius:50%; width:22px; height:22px; text-align:center; line-height:22px; font-weight:bold; border:1px solid #333; font-size:12px;">${id}</div>` }) 
            }).addTo(highlightLayers);
...
map.on('mousemove', (e) => {
    ...
    if (currentTool === 'point' && placementState === 'location_placed') { 
        ...
        if (tempMeasureLine) map.removeLayer(tempMeasureLine); 
        tempMeasureLine = L.polyline([measureStart, e.latlng], {color: '#dc3545', dashArray: '5, 10'})
            .addTo(map)
            .bindTooltip(formatDistance(map.distance(measureStart, e.latlng)), {permanent: false})
